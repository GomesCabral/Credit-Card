...
const findInvalidCards = (cCard) =>{
  let invalid = [];

  for(let i = 0; i < cCard.length; i++){
    if(!validateCred(cCard[i])){
      invalid.push(cCard[i]);
    }
  }
  return invalid;
}
...
function idInvalidCardCompanies(arr){
  ...
  for(let i = 0; i < arr.length; i++){
    if(arr[i][0] === 3){
      if(!ccCompanies.includes('Amex')){
        ccCompanies.push('Amex');
      }
    }else if(arr[i][0] === 4){
      if(!ccCompanies.includes('Visa')){
        ccCompanies.push('Visa');
      }
    }else if(arr[i][0] === 5){
      if(!ccCompanies.includes('Mastercard')){
        ccCompanies.push('Mastercard');
      }
    }else if(arr[i][0] === 6){
      if(!ccCompanies.includes('Discover')){
